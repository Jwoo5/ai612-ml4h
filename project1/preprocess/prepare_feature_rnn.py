...
with tqdm(total = len(train_charts) + len(test_charts)) as pbar:
    for icu, label in zip(train_charts, train_y):
        ...
        before = chart_times[0]

        for time, item, value in zip(chart_times, items, valuenums):
            if item in codebook:
                if timedelta() < time - before:
                    elapsed += time - before
                feature += f"{elapsed.seconds / 60}:{item}:{value} "
                before = time
...
np.save('../20218078/X_train_rnn', train_features)
np.save('../20218078/X_test_rnn', test_features)
